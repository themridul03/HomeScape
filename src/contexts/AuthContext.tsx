...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [token, setToken] = useState<string | null>(localStorage.getItem('HomeScape_token'));
  const [isLoading, setIsLoading] = useState(true);

  // On mount, check if token exists and is valid
  useEffect(() => {
    const storedToken = localStorage.getItem('HomeScape_token');
    const storedUser = localStorage.getItem('HomeScape_user');
    if (storedToken && storedUser) {
      try {
        ...
        localStorage.removeItem('HomeScape_user');
      }
    }
    setIsLoading(false);
  }, []);
